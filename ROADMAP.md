...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   PHP 原生方式                    OYTAPHP 创新方式                           │
│   ┌─────────────────┐            ┌─────────────────┐                       │
│   │ Generator/yield │            │ oyta_stream_*   │                       │
│   │ Fiber 协程      │     VS     │ oyta_coroutine  │                       │
│   │ 类继承          │            │ 配置式 API      │                       │
│   │ 标准语法        │            │ 高性能替代      │                       │
│   └─────────────────┘            └─────────────────┘                       │
│          │                              │                                   │
...
│   ┌─────────────────────────────────────────────────────────────────┐     │
│   │                      用户可以选择使用                            │     │
│   │   - 想用 PHP 原生 Generator？可以！                              │     │
│   │   - 想用 OYTAPHP 高性能方式？也可以！                            │     │
│   └─────────────────────────────────────────────────────────────────┘     │
│                                                                             │
│   结论：两种方式并存，互不冲突，用户自由选择                                │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 接口类型 | 说明 | 特点 |
|---------|------|------|
| **PHP 原生方式** | 遵循 PHP 语法规范，兼容现有代码 | 兼容性好、学习成本低、符合标准 |
| **OYTAPHP 创新方式** | 基于 Rust 高性能实现 | 性能更高、API 简化、功能增强 |

### 依赖关系

- **PHP 原生方式**：依赖解释器对 PHP 8.x 特性的支持（Generator、Fiber、Enum 等）
- **OYTAPHP 创新方式**：基于 Rust 底层实现，解释器当前即可支持

---

## 零、解释器 PHP 8.x 特性支持（前置任务）

### 0.1 功能描述

为支持 PHP 原生方式的创新功能接口，解释器需要增强对 PHP 8.x 新特性的支持。这是实现"两套接口并存"原则的前置条件。

### 0.2 需要支持的特性

#### 0.2.1 Generator / yield（PHP 5.5+）
...
// Fiber 状态
$fiber->isStarted();   // 是否已启动
$fiber->isSuspended(); // 是否已挂起
$fiber->isTerminated();// 是否已终止
$fiber->getReturn();   // 获取返回值
...
#### 0.2.5 First-class Callable（PHP 8.1+）
...
1. **mod.rs 文件规范**：`mod.rs` 文件只允许用于模块的引入，不允许在 `mod.rs` 文件内编写任何功能代码
2. **功能完整实现**：功能需要 100% 完整实现，不允许省略或简化任何功能代码和业务逻辑
3. **代码注释规范**：每一行代码都需要有详细的中文注释，说明代码的作用和目的
4. **模块拆分规范**：模块功能过多文件过大的需要将功能进行拆分，在文件目录中以子模块形式实现完整功能

### 0.6 实现优先级

| 优先级 | 特性 | 预计工作量 | 说明 |
|--------|------|-----------|------|
| P0 | Generator / yield | 1 周 | gRPC 流式、数据流处理依赖 |
| P0 | Fiber 协程 | 1 周 | 原生协程调度器依赖 |
| P1 | 枚举类型 | 3 天 | 类型系统增强 |
| P1 | 只读属性 | 2 天 | 属性系统增强 |
| P2 | First-class Callable | 2 天 | 函数式编程增强 |
| P2 | 析构器 | 1 天 | 语法糖 |
...
统一异步编程接口，简化并发任务处理。提供 PHP 原生 Fiber 方式和 OYTAPHP 高性能方式两种接口。

### 3.2 PHP 接口设计

#### 3.2.1 PHP 原生方式（依赖 Fiber 支持）

```php
<?php

// ═══════════════════════════════════════════════════════════════
// 方式一：PHP 原生 Fiber 方式（需要解释器支持 Fiber）
// ═══════════════════════════════════════════════════════════════

// 使用 Fiber 创建协程
$fiber = new Fiber(function() {
    $data = async_fetch();
    $result = Fiber::suspend($data);  // 挂起等待
...
// 恢复协程
$finalResult = $fiber->resume($processedData);
...
#### 3.2.2 OYTAPHP 高性能方式（当前可用）

```php
<?php

// ═══════════════════════════════════════════════════════════════
// 方式二：OYTAPHP 高性能方式（基于 Rust 实现，当前可用）
// ═══════════════════════════════════════════════════════════════
...
// 创建协程（回调方式）
$coroutineId = oyta_go(function() {
    $data = oyta_async_fetch();  // 异步获取
    return process($data);
});

// 等待协程结果
$result = oyta_await($coroutineId, timeout: 5000);

// 协程通道
$channel = oyta_channel(100);  // 缓冲区大小 100
oyta_go(function() use ($channel) {
    $channel->send(['data' => 1]);
});
oyta_go(function() use ($channel) {
    $data = $channel->recv();
...
| 特性 | PHP 原生 Fiber | OYTAPHP 协程 |
|------|---------------|-------------|
| 语法 | 标准 PHP 语法 | 函数式 API |
| 性能 | 中等 | 高（Rust 实现） |
| 兼容性 | PHP 8.1+ | 当前可用 |
| 学习成本 | 较高 | 较低 |
| 调试 | 原生支持 | 需要工具 |
...
PHP 不擅长处理实时数据流。基于 Rust 可以实现高性能的流式数据处理引擎，支持窗口聚合、复杂事件处理（CEP）、实时分析等场景。提供 PHP 原生 Generator 方式和 OYTAPHP 高性能方式两种接口。
...
#### 14.3.1 PHP 原生方式（依赖 Generator 支持）

```php
<?php

// ═══════════════════════════════════════════════════════════════
// 方式一：PHP 原生 Generator 方式（需要解释器支持 Generator）
// ═══════════════════════════════════════════════════════════════

// 定义数据流生成器
function kafkaStream(string $topic, array $config): Generator {
    ...
    while (true) {
        ...
        if ($message !== null) {
            yield json_decode($message->payload, true);
        }
    }
}

// 流式处理管道（Generator 链式调用）
function filterStream(Generator $source, callable $filter): Generator {
...
function mapStream(Generator $source, callable $mapper): Generator {
    foreach ($source as $item) {
        yield $mapper($item);
    }
}

function windowStream(Generator $source, int $size): Generator {
    $batch = [];
    foreach ($source as $item) {
        $batch[] = $item;
        if (count($batch) >= $size) {
...
// 使用示例
$source = kafkaStream('user-events', ['brokers' => 'localhost:9092']);

$filtered = filterStream($source, fn($e) => $e['type'] === 'purchase');
$mapped = mapStream($filtered, fn($e) => ['user_id' => $e['user_id'], 'amount' => $e['amount']]);
$windowed = windowStream($mapped, 100);

foreach ($windowed as $batch) {
    $sum = array_sum(array_column($batch, 'amount'));
    Cache::set('realtime:purchase_sum', $sum);
}

// 复杂事件处理（CEP）使用 Generator
function detectPattern(Generator $source, array $pattern): Generator {
    $buffer = [];
    foreach ($source as $event) {
        $buffer[] = $event;
        
        // 检测模式：login -> purchase -> logout
        if (count($buffer) >= 3) {
            if ($buffer[count($buffer)-3]['type'] === 'login' &&
                $buffer[count($buffer)-2]['type'] === 'purchase' &&
                $buffer[count($buffer)-1]['type'] === 'logout') {
                yield array_slice($buffer, -3);
            }
        }
    }
}
...
// 配置式 API
oyta_stream_pipeline('user-analytics', [
    'source' => [
        'type' => 'kafka',
        'topic' => 'user-events',
    ...
    'operators' => [
        ['type' => 'filter', 'condition' => "type === 'purchase'"],
        ['type' => 'map', 'fields' => ['user_id', 'amount']],
        ['type' => 'window', 'size' => 60, 'unit' => 'seconds'],
        ['type' => 'aggregate', 'function' => 'sum', 'field' => 'amount'],
    ],
    'sink' => [
        'type' => 'redis',
        'key' => 'realtime:purchase_sum',
    ],
]);

// 链式 API
oyta_stream('user-events')
    ->source('kafka', ['topic' => 'user-events', 'brokers' => ['kafka1:9092']])
    ->filter(fn($e) => $e['type'] === 'purchase')
    ->map(fn($e) => ['user_id' => $e['user_id'], 'amount' => $e['amount']])
    ->window(60, 'seconds')
    ->aggregate('sum', 'amount')
    ->sink('redis', 'realtime:stats');
...
oyta_cep_detect([
    ...
    'pattern' => [
        ['event' => 'login'],
        ['event' => 'purchase', 'condition' => 'amount > 1000'],
        ['event' => 'logout'],
    ],
    'within' => 3600,  // 1小时内
...
oyta_stream_topology('user-analytics')
    ...
    ->transform('filter_spam', fn($e) => !$e['is_spam'])
    ->transform('extract_fields', fn($e) => ['id' => $e['id'], 'type' => $e['type']])
...
| 特性 | PHP 原生 Generator | OYTAPHP 流处理 |
|------|-------------------|----------------|
| 语法 | 标准 PHP 语法 | 配置式/链式 API |
| 性能 | 中等 | 高（Rust 实现） |
| 内存 | 需要手动管理 | 自动优化 |
| 并行 | 单线程 | 多线程并行 |
| 容错 | 需要自己实现 | 内置支持 |
...
└── stream/
    ...
    ├── sink.rs             # 输出目标
    ├── topology.rs         # 处理拓扑
    └── generator_bridge.rs # Generator 桥接（可选）
```
...
PHP 不支持真正的多线程和 Actor 模型。基于 Rust 可以实现高性能的 Actor 并发模型，简化并发编程，支持分布式 Actor 集群。提供 PHP 原生类继承方式和 OYTAPHP 配置式方式两种接口。
...
#### 15.3.1 PHP 原生方式（依赖类继承支持）

```php
<?php

// ═══════════════════════════════════════════════════════════════
// 方式一：PHP 原生类继承方式（需要解释器支持）
// ═══════════════════════════════════════════════════════════════
...
class CounterActor extends OytaActor {
    ...
    public function receive(mixed $message): mixed {
        return match ($message['type']) {
            'increment' => $this->count += $message['value'] ?? 1,
            'get' => $this->count,
            'reset' => $this->count = 0,
            default => null,
        };
    ...
    public function preRestart(Throwable $reason): void {
        Log::warning("CounterActor restarting", ['reason' => $reason->getMessage()]);
    }
    
    public function postRestart(Throwable $reason): void {
        Log::info("CounterActor restarted");
    }
}
...
#### 15.3.2 OYTAPHP 配置式方式（当前可用）

```php
<?php

// ═══════════════════════════════════════════════════════════════
// 方式二：OYTAPHP 配置式方式（基于 Rust 实现，当前可用）
// ═══════════════════════════════════════════════════════════════

// 创建 Actor（配置方式）
oyta_actor_create('counter-1', [
    'state' => ['count' => 0],
    'handlers' => [
        'increment' => function(array $state, array $message): array {
            $state['count'] += $message['value'] ?? 1;
            return ['state' => $state, 'reply' => $state['count']];
        },
        'get' => function(array $state, array $message): array {
            return ['state' => $state, 'reply' => $state['count']];
        },
        'reset' => function(array $state, array $message): array {
            $state['count'] = 0;
            return ['state' => $state, 'reply' => true];
        },
    ],
    'lifecycle' => [
        'preStart' => function() { Log::info("Actor started"); },
        'postStop' => function() { Log::info("Actor stopped"); },
    ],
]);

// 发送消息（异步）
oyta_actor_tell('counter-1', ['type' => 'increment', 'value' => 5]);

// 发送消息并等待回复（同步）
$result = oyta_actor_ask('counter-1', ['type' => 'get'], timeout: 5000);

// 创建 Actor 路由
oyta_actor_router('counter-router', [
    'type' => 'pool',
    'actor_config' => [
        'state' => ['count' => 0],
        'handlers' => [
...
// 配置监督策略
oyta_actor_supervise('counter-1', [
    'strategy' => 'one_for_one',
    'max_restarts' => 3,
    'within_time' => 60,
    'on_failure' => 'restart',  // restart / stop / resume
]);

// 分布式 Actor 集群
oyta_actor_cluster_config([
    'name' => 'my-cluster',
    'nodes' => [
        ['host' => 'node1', 'port' => 9000],
        ['host' => 'node2', 'port' => 9000],
        ['host' => 'node3', 'port' => 9000],
    ],
    'strategy' => 'consistent_hash',
    'replication' => 2,
]);
...
// 远程 Actor 调用
$result = oyta_actor_ask('counter-1@node1', ['type' => 'get']);

// Actor 状态持久化
oyta_actor_persist('counter-1', [
    'storage' => 'redis',
    'key' => 'actor:counter-1:state',
    'snapshot_interval' => 100,  // 每 100 条消息快照一次
]);

// Actor 统计信息
$stats = oyta_actor_stats('counter-1');
// 返回: ['messages_processed' => 1000, 'errors' => 0, 'uptime' => 3600]
```

### 15.4 两种方式对比

| 特性 | PHP 原生类继承 | OYTAPHP 配置式 |
|------|--------------|----------------|
| 语法 | 标准 PHP OOP | 配置式 API |
| 类型安全 | 强类型 | 弱类型 |
| IDE 支持 | 完整 | 有限 |
| 性能 | 中等 | 高 |
| 状态管理 | 类属性 | 显式 state |
...
PHP 的 gRPC 支持需要依赖扩展且性能较差。基于 Rust 可以实现高性能的原生 gRPC 支持，包括流式 RPC、双向流、拦截器等完整功能。提供 PHP 原生 Generator 方式和 OYTAPHP 高性能方式两种接口。
...
#### 17.3.1 PHP 原生方式（依赖 Generator 支持）
...
class UserServiceGrpc {
    ...
    // 服务端流式 RPC（回调方式）
    #[Grpc\Method(name: 'ListUsersV2', type: 'server_stream')]
    public function ListUsersV2(ListUsersRequest $request, GrpcStream $stream): void {
        foreach (User::chunk(100) as $users) {
            foreach ($users as $user) {
                $stream->send(new GetUserResponse([
                    'id' => $user->id,
                    'name' => $user->name,
                ]));
            }
    ...
    // 客户端流式 RPC（回调方式）
    #[Grpc\Method(name: 'UploadData', type: 'client_stream')]
    public function UploadData(GrpcStreamReader $reader): UploadResponse {
        $count = 0;
        while ($req = $reader->recv()) {
            $count++;
            // 处理客户端流
        }
        return new UploadResponse(['count' => $count]);
    ...
    // 双向流式 RPC（回调方式）
    #[Grpc\Method(name: 'Chat', type: 'bidi_stream')]
    public function Chat(GrpcStreamReader $reader, GrpcStream $writer): void {
        while ($req = $reader->recv()) {
            $writer->send(new ChatResponse([
...
// 客户端流式调用
$stream = $client->clientStream('user.UserService/UploadData');
$stream->send(['data' => 'chunk1']);
$stream->send(['data' => 'chunk2']);
$response = $stream->finish();

// 双向流调用
$bidiStream = $client->bidiStream('chat.ChatService/Chat');
$bidiStream->send(['message' => 'Hello']);
$response = $bidiStream->recv();
$bidiStream->close();
```

### 17.4 两种方式对比

| 特性 | PHP 原生 Generator | OYTAPHP 流式 API |
|------|-------------------|------------------|
| 语法 | 标准 PHP 语法 | 回调/数组方式 |
| 流式支持 | 原生 yield | 回调/迭代器 |
| 性能 | 中等 | 高 |
| 内存 | 惰性求值 | 可控缓冲 |
| 兼容性 | PHP 5.5+ | 当前可用 |
| 调试 | 容易 | 需要工具 |
...
| 优先级 | 特性 | 预计工作量 | 价值 | 说明 |
|--------|------|-----------|------|------|
| P0 | Generator / yield | 1 周 | 极高 | gRPC 流式、数据流处理依赖 |
| P0 | Fiber 协程 | 1 周 | 极高 | 原生协程调度器依赖 |
| P1 | 枚举类型 | 3 天 | 高 | 类型系统增强 |
| P1 | 只读属性 | 2 天 | 中 | 属性系统增强 |
| P2 | First-class Callable | 2 天 | 中 | 函数式编程增强 |
| P2 | 析构器 | 1 天 | 低 | 语法糖 |
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| 3.1.0 | 2026 | 为所有功能添加硬性要求（mod.rs规范、完整实现、中文注释、模块拆分） |
| 3.0.0 | 2026 | 新增"两套接口并存"设计原则；新增解释器 PHP 8.x 特性支持章节；为协程、数据流、Actor、gRPC 添加两种接口设计 |
| 2.0.0 | 2026 | 新增 SIMD、序列化、沙箱、HTTP/3、数据流、Actor、智能路由、gRPC、热更新等功能 |
| 1.0.0 | 2024 | 初始版本，包含九大创新功能规划 |
